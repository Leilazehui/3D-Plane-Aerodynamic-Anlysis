...
In Xflr5 Aanalysis, the data of CL, CD, CM and other parameters when the angle of the aircraft of 2 is considered. The final lift force with the lift equation: L = (rho)*(Cl)*(V^2)*(WingArea)*1/2 and drag force with the drag equation:D = (rho)*(Cd)*(V^2)*(Wingarea)*1/2 are computed.
